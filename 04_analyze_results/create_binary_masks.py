...
output_folder = '/Volumes/SSD/Uni/PhD/BINARY MASKS/TCGA_COADREAD_CSV/'
...
# cutoff values for our variables
cutoff_values = {
...
for file in os.listdir(csv_folder):
        if file.endswith(".csv") and not file.startswith("image_level_data"):
            ...
            # Remove all the variables except the ones we are interested in
            #variables_to_visualize = {var: variables_to_visualize[var] for var in ['normalized_cells_in_stroma_lymphocyte', 'normalized_cells_in_tumor_lymphocyte', 'normalized_cells_in_stroma_eosinophil', 'normalized_cells_in_stroma_neutrophil', 'percentage_cells_in_stroma_connective']}
            variables_to_visualize = {var: variables_to_visualize[var] for var in ['percentage_cells_in_stroma_connective']}
            ...
            for var_name in variables_to_visualize:
                ...
                result = file[:-15] + '_' + var_name + '.png'
                if result in os.listdir(heatmap_folder_blurred):
                    print('      heatmap already exists')
                    continue

                if var_name in cutoff_values:
                    cutoff_value = cutoff_values[var_name]
                else:
                    print('Error: No cutoff value found for variable ' + var_name)

                # Take number for patch, normalize it and create a corresponding heatmap color
                color_patches = []
                for value in variables_to_visualize[var_name]:
                    if value > cutoff_value:
                        color_patches.append((255, 0, 0))
                    else:
                        color_patches.append((0, 0, 255))
