...
input_WSI_folder = '/media/vmitchell/ssd8tb/01_CRC_UKK_Cut_NEW_2017_2019/'
input_mask_folder = '/media/vmitchell/ssd8tb/01_CRC_UKK_Cut_NEW_2017_2019_MAPS/mask_filter/'
input_geojson_folder = '/media/vmitchell/ssd8tb/01_CRC_UKK_Cut_NEW_2017_2019_NUCLEI/'
input_qc_folder = '/media/vmitchell/ssd8tb/01_CRC_UKK_Cut_NEW_2017_2019_QC/tis_det_mask/'
input_thumbnail_folder = '/media/vmitchell/ssd8tb/01_CRC_UKK_Cut_NEW_2017_2019_MAPS/tis_det_thumbnail/'
include_image_folder = ''

output_path = '/media/vmitchell/ssd8tb/01_CRC_UKK_Cut_NEW_2017_2019_CSV/'
...
for filename in os.listdir(input_WSI_folder):
    if filename.endswith('.ndpi') or filename.endswith('.svs') or filename.endswith('.tiff'):
        ...
        if find_file_containing_string(output_path + 'csv/', os.path.splitext(filename)[0]):
        ...
        cells_in_stroma_lymphocyte = count_nuclei_in_geometry_collection(rtrees_patches_lymphocyte, poly_stroma_map_patches)
        cells_in_stroma_plasma = count_nuclei_in_geometry_collection(rtrees_patches_plasma, poly_stroma_map_patches)
        cells_in_stroma_neutrophil = count_nuclei_in_geometry_collection(rtrees_patches_neutrophil, poly_stroma_map_patches)
        cells_in_stroma_eosinophil = count_nuclei_in_geometry_collection(rtrees_patches_eosinophil, poly_stroma_map_patches)
        cells_in_stroma_connective = count_nuclei_in_geometry_collection(rtrees_patches_connective, poly_stroma_map_patches)

        cells_in_stroma_total = sum_of_lists(
            cells_in_stroma_lymphocyte,
            cells_in_stroma_plasma,
            cells_in_stroma_neutrophil,
            cells_in_stroma_eosinophil,
            cells_in_stroma_connective
        )

        # Calculate the number of cells in tumor areas
        ...
        # Calculate cells normalized based on area of tumor
        normalized_cells_in_tumor_epithelial = normalize_cell_count_in_area(cells_in_tumor_epithelial, area_patches_tumor)
        normalized_cells_in_tumor_lymphocyte = normalize_cell_count_in_area(cells_in_tumor_lymphocyte, area_patches_tumor)

        # Calculate cells normalized based on area of stroma
        normalized_cells_in_stroma_lymphocyte = normalize_cell_count_in_area(cells_in_stroma_lymphocyte, area_patches_stroma)
        normalized_cells_in_stroma_plasma = normalize_cell_count_in_area(cells_in_stroma_plasma, area_patches_stroma)
        normalized_cells_in_stroma_neutrophil = normalize_cell_count_in_area(cells_in_stroma_neutrophil, area_patches_stroma)
        normalized_cells_in_stroma_eosinophil = normalize_cell_count_in_area(cells_in_stroma_eosinophil, area_patches_stroma)
        normalized_cells_in_stroma_connective = normalize_cell_count_in_area(cells_in_stroma_connective, area_patches_stroma)

        # Calculate cells normalized on total number of cells
        percentage_cells_in_stroma_lymphocyte = calculate_ratio(cells_in_stroma_lymphocyte, cells_in_stroma_total)
        percentage_cells_in_stroma_plasma = calculate_ratio(cells_in_stroma_plasma, cells_in_stroma_total)
        percentage_cells_in_stroma_neutrophil = calculate_ratio(cells_in_stroma_neutrophil, cells_in_stroma_total)
        percentage_cells_in_stroma_eosinophil = calculate_ratio(cells_in_stroma_eosinophil, cells_in_stroma_total)
        percentage_cells_in_stroma_connective = calculate_ratio(cells_in_stroma_connective, cells_in_stroma_total)

        # Calculate ratios
        ratio_lympocyte_to_plasma = calculate_ratio(cells_in_stroma_lymphocyte, cells_in_stroma_plasma)
        ratio_lympocyte_to_connective = calculate_ratio(cells_in_stroma_lymphocyte, cells_in_stroma_connective)
        ratio_plasma_to_connective = calculate_ratio(cells_in_stroma_plasma, cells_in_stroma_connective)
        ratio_lymphocyte_and_plasma_to_connective = calculate_ratio(sum_of_lists(cells_in_stroma_lymphocyte, cells_in_stroma_plasma), cells_in_stroma_connective)
        ratio_lymphocyte_and_plasma_and_neutrophil_and_eosinophil_to_connective = calculate_ratio(sum_of_lists(cells_in_stroma_lymphocyte, cells_in_stroma_plasma, cells_in_stroma_neutrophil, cells_in_stroma_eosinophil), cells_in_stroma_connective)

        ratio_eosinophil_to_lymphocyte = calculate_ratio(cells_in_stroma_eosinophil, cells_in_stroma_lymphocyte)
        ratio_eosinophil_to_plasma = calculate_ratio(cells_in_stroma_eosinophil, cells_in_stroma_plasma)
        ratio_eosinophil_to_connective = calculate_ratio(cells_in_stroma_eosinophil, cells_in_stroma_connective)
        ratio_eosinophil_to_lymphocyte_and_plasma_and_connective = calculate_ratio(cells_in_stroma_eosinophil, sum_of_lists(cells_in_stroma_lymphocyte, cells_in_stroma_plasma, cells_in_stroma_connective))
        ratio_eosinophil_to_lymphocyte_and_plasma = calculate_ratio(cells_in_stroma_eosinophil, sum_of_lists(cells_in_stroma_lymphocyte, cells_in_stroma_plasma))

        ratio_neutrophil_to_lymphocyte = calculate_ratio(cells_in_stroma_neutrophil, cells_in_stroma_lymphocyte)
        ratio_neutrophil_to_plasma = calculate_ratio(cells_in_stroma_neutrophil, cells_in_stroma_plasma)
        ratio_neutrophil_to_connective = calculate_ratio(cells_in_stroma_neutrophil, cells_in_stroma_connective)
        ratio_neutrophil_to_lymphocyte_and_plasma_and_connective = calculate_ratio(cells_in_stroma_neutrophil, sum_of_lists(cells_in_stroma_lymphocyte, cells_in_stroma_plasma, cells_in_stroma_connective))
        ratio_neutrophil_to_lymphocyte_and_plasma = calculate_ratio(cells_in_stroma_neutrophil, sum_of_lists(cells_in_stroma_lymphocyte, cells_in_stroma_plasma))

        # Calculate global variables
        total_cell_count_lymphocyte = sum(cells_in_stroma_lymphocyte)
        total_cell_count_plasma = sum(cells_in_stroma_plasma)
        total_cell_count_neutrophil = sum(cells_in_stroma_neutrophil)
        total_cell_count_eosinophil = sum(cells_in_stroma_eosinophil)
        total_cell_count_connective = sum(cells_in_stroma_connective)
        total_cell_count_total = sum(cells_in_stroma_total)

        total_area_tumor = sum(area_patches_tumor)
        total_area_stroma = sum(area_patches_stroma)
        total_area_total = sum(area_patches_total)
        ...
        all_patch_variables = [
            ...
            'normalized_cells_in_tumor_epithelial',
            'normalized_cells_in_tumor_lymphocyte',
            'normalized_cells_in_stroma_lymphocyte',
            'normalized_cells_in_stroma_plasma',
            'normalized_cells_in_stroma_neutrophil',
            'normalized_cells_in_stroma_eosinophil',
            'normalized_cells_in_stroma_connective',
            'percentage_cells_in_stroma_lymphocyte',
            'percentage_cells_in_stroma_plasma',
            'percentage_cells_in_stroma_neutrophil',
            'percentage_cells_in_stroma_eosinophil',
            'percentage_cells_in_stroma_connective',
            'ratio_lympocyte_to_plasma',
            'ratio_lympocyte_to_connective',
            'ratio_plasma_to_connective',
            'ratio_lymphocyte_and_plasma_to_connective',
            'ratio_lymphocyte_and_plasma_and_neutrophil_and_eosinophil_to_connective',
            'ratio_eosinophil_to_lymphocyte',
            'ratio_eosinophil_to_plasma',
            'ratio_eosinophil_to_connective',
            'ratio_eosinophil_to_lymphocyte_and_plasma_and_connective',
            'ratio_eosinophil_to_lymphocyte_and_plasma',
            'ratio_neutrophil_to_lymphocyte',
            'ratio_neutrophil_to_plasma',
            'ratio_neutrophil_to_connective',
            'ratio_neutrophil_to_lymphocyte_and_plasma_and_connective',
            'ratio_neutrophil_to_lymphocyte_and_plasma'
        ]

        all_universal_variables = [
            'total_cell_count_lymphocyte',
            'total_cell_count_plasma',
            'total_cell_count_neutrophil',
            'total_cell_count_eosinophil',
            'total_cell_count_connective',
            'total_cell_count_total',
            'total_area_tumor',
            'total_area_stroma',
            'total_area_total'
        ]
        ...
        # Add the global variables to the combined CSV file, creating it if it doesn't already exist
        with open(output_path_global_csv, mode='a' if csv_file_exists else 'w', newline='') as file:
            writer = csv.writer(file)

            if not csv_file_exists:
                writer.writerow(['image_name'] + all_universal_variables)

            variable_values = [universal_variables_dict[var_name] for var_name in all_universal_variables]

            # Write a row for this slide
            writer.writerow([filename] + variable_values)

        print('  generating json output')

        # Export data to a json file
        with open(output_path_json, 'w') as f:
            json.dump(combined_variable_dict, f, indent=4)  # indent is optional, but it makes the file human-readable

        print('  generating heatmap outputs')

        # Export labelled image
        #patch_size_mask = chunks_list[0].size[0]
        label_patches_images = [create_image_with_number(patch_size_mask, number, display_number_size) for number in significant_indexes]
        label_reconstructed_image = reconstruct_image(mask_image, significant_indexes, label_patches_images, patches_downscaled)
        label_overlayed_image = overlay_images(label_reconstructed_image, thumbnail_image, darkening_factor)
        label_overlayed_image.save(output_path_heatmap + 'label' + '.png', 'PNG')

        # Export image files which will be used to generate the heatmap images from the CSV files

        # Export mask image as a bw mask, where tissue is white
        mask_universal_image = mask_image.point(lambda x: 255 if x != 0 else 0, '1')
        mask_universal_image.save(output_path_heatmap + 'universal_mask' + '.png', 'PNG')

        # Export thumbnail_image
        thumbnail_image.save(output_path_heatmap + 'thumbnail' + '.png', 'PNG')